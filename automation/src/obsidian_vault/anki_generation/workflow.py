...
from obsidian_vault.anki_generation.agent import CardGenerationAgent
from obsidian_vault.anki_generation.duplicate_checker import DuplicateChecker
from obsidian_vault.anki_generation.firecrawl import FirecrawlClient, FirecrawlError
from obsidian_vault.anki_generation.models import WorkflowResult
from obsidian_vault.anki_generation.note_builder import NoteWriter
...
class AnkiIngestionWorkflow:
    """Coordinates the Anki ingestion pipeline."""
    ...
    def run(self, url: str, config: WorkflowConfig | None = None) -> WorkflowResult:
        ...
        logger.info("Starting Anki ingestion workflow", url=url, dry_run=workflow_config.dry_run)
        ...
        logger.success("Anki ingestion workflow complete", created=len(created_paths))
        return result
