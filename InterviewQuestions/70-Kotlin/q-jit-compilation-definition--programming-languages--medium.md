...
**JIT (Just-In-Time compilation)** — это технология, при которой **байткод компилируется в машинный код во время выполнения программы** (а не до запуска).

## Как это работает

### Традиционная компиляция vs JIT

**Ahead-of-Time (AOT) компиляция:**

```
Исходный код → Компилятор → Машинный код → Выполнение
  (.c, .cpp)                  (нативный)      (запуск)

Пример: C, C++, Rust
- Компиляция до распространения
- Платформо-специфичный бинарник
- Быстрое выполнение (уже машинный код)
```
...
Пример: Python (CPython), базовый JavaScript
- Нет шага компиляции
- Медленное выполнение (интерпретируется каждый раз)
- Платформо-независимый исходный код
```

**JIT компиляция:**

```
Исходный код → Байткод → JIT компилятор → Машинный код → Выполнение
  (.java)      (.class)    (runtime)        (нативный)     (запуск)

Пример: Java, Kotlin, C#, современный JavaScript
- Сначала компилируется в байткод (портируемый)
- JIT компилирует горячий код в машинный код во время выполнения
- Адаптивная оптимизация на основе реального использования
```
