...
Виртуальное время - ключевая функция тестирования корутин, которая позволяет тестам выполняться мгновенно, симулируя прохождение времени.
...
fun `advanceTimeBy продвигает время точно`() = runTest {
    ...
    launch {
        delay(100)
        events.add("Событие в 100ms")
    }

    advanceTimeBy(150)
    assertEquals(listOf("Событие в 100ms"), events)
    assertEquals(150, currentTime)
}
```
...
Выполняет все ожидающие корутины до завершения:
...
Выполняет только текущие задачи, не продвигая время:
