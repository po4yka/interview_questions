---
anki_cards:
- slug: q-actor-pattern--kotlin--hard-0-en
  language: en
  difficulty: 0.7
  front: What is the Actor pattern in Kotlin coroutines?
  back: |
    **Actor pattern** - concurrency model where actors are isolated units processing messages sequentially.
    **Key concepts:**
    - Private encapsulated state
    - Message passing via Channel
    - Sequential processing (no locks needed)
    - Thread-safe by design
    Use when: mutable state with concurrent access, complex state updates, event-driven systems.
  tags:
  - Kotlin
  - coroutines
  - concurrency
  - actor
- slug: q-actor-pattern--kotlin--hard-0-ru
  language: ru
  difficulty: 0.7
  front: Chto takoe pattern Actor v korutinakh Kotlin?
  back: |
    **Actor pattern** - model parallelizma, gde aktory - izolirovannye edinitsy, obrabatyvayushchie soobshcheniya posledovatelno.
    **Klyuchevye kontseptsii:**
    - Privatnoe inkapsulirovannoe sostoyanie
    - Peredacha soobshcheniy cherez Channel
    - Posledovatelnaya obrabotka (bez blokirovok)
    - Thread-safe po dizaynu
    Ispolzovat kogda: izmenyaemoe sostoyanie s konkurentnym dostupom.
  tags:
  - Kotlin
  - coroutines
  - concurrency
