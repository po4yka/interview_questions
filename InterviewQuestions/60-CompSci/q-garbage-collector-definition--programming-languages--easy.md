...
**English**: What is a garbage collector?
...
Сборщик мусора (Garbage Collector, GC) — это механизм управления памятью, который автоматически освобождает память, выделенную для объектов, которые больше не используются в программе. Эта функция присутствует во многих современных языках программирования, где не требуется явно управлять памятью как это делается в языках например C или C++. Как он работает: Работает в фоновом режиме периодически сканируя память программы на предмет объектов которые больше не доступны для использования. Доступность объекта обычно определяется по наличию живых ссылок на него. Если на объект нет активных ссылок то считается что он больше не нужен и память которую он занимает может быть освобождена. Существуют разные алгоритмы сборки мусора но многие из них используют следующие базовые принципы: Пометка (Marking) - Сканирует объекты доступные из корневого набора и помечает все объекты которые можно достичь как живые. Очистка (Sweeping) - Затем сборщик проходит по всей куче heap памяти и освобождает память занимаемую объектами которые не были помечены как живые. Преимущества: Уменьшение ошибок управления памятью. Простота разработки. Недостатки: Непредсказуемость и накладные расходы.
