...
2. **HTTP/1.1 или HTTP/2**: используется стандартный HTTP (SSE может работать поверх HTTP/2 как стриминговый ответ; поведение зависит от реализации)
...
class SseClient(
     ...
     * Включает внутреннее переподключение с backoff; важно
        ...
        suspend fun attemptOnce(): Boolean {
            ...
            return try {
                ...
                if (!response.isSuccessful) {
                    ...
                    // Ошибка сервера — пробуем ещё раз
                    true
                } else {
    ...
    fun connectWithEventSource(
        ...
        val listener = object : EventSourceListener() {
            ...
            override fun onFailure(
                ...
                // Переподключение/завершение контролируется вызывающей стороной.
            }
        }
...
/**
 * Парсер формата SSE-сообщений (упрощённый, не покрывает все крайние случаи спецификации)
 */
class SseParser {
    ...
    fun parseLine(line: String): SseEvent.Message? {
        return when {
            line.isEmpty() -> {
                // Пустая строка сигнализирует завершение события
    ...
    private fun parseField(line: String) {
        ...
        if (colonIndex == -1) {
            // В этом упрощённом парсере строка без ':' игнорируется
            return
        }
...
class TypedSseClient(
    ...
    /**
     * Подписка на конкретный тип события с десериализацией данных
     */
    ...
    /**
     * Заглушка: как можно было бы обрабатывать несколько типов событий.
     */
...
/**
 * Обёртка над типизированными SSE-событиями
 */
...
/**
 * Обработчик конкретного типа события
 */
...
### Сравнение SSE, WebSocket и Polling (полная таблица)

| Характеристика | SSE | WebSocket | `Long` Polling | `Short` Polling |
|----------------|-----|-----------|----------------|-----------------|
| Направление | Сервер → Клиент | Двунаправленный | Сервер → Клиент | Сервер → Клиент |
| Протокол | HTTP | WebSocket (`ws://`, `wss://`) | HTTP | HTTP |
| Соединение | Постоянное | Постоянное | Долго живущий запрос | Много коротких запросов |
| Сложность | Низкая | Средняя | Средняя | Низкая |
| Поддержка в браузерах | Нативный `EventSource` | Нативный `WebSocket` | Реализация вручную | Простые запросы |
| Переподключение | Встроено в `EventSource` / вручную в клиентах | Вручную на клиенте | Вручную на клиенте | Не применимо |
...
| Прокси/фаерволы | Обычно работает хорошо | Возможны проблемы | Обычно работает хорошо | Обычно работает хорошо |
| HTTP/2 | Может работать как HTTP/2 streaming | Поддержка через upgrade (RFC 8441, зависит от реализации) | Ограниченные выгоды | Ограниченные выгоды |
| ID событий | Встроенное поле | Нужно реализовать самостоятельно | Нужно реализовать самостоятельно | Не применимо |
| Логика retry | Через `retry` и `Last-Event-ID` | Вручную | Вручную | Не применимо |

### Матрица выбора: когда что использовать

Используйте SSE, когда:
- Нужны односторонние обновления сервер → клиент.
- Важна простота реализации поверх HTTP.
- Важна поддержка `EventSource` в браузерах.
- Нужно возобновление потока по `Last-Event-ID`.
- Передаются текстовые/JSON-данные.

Используйте WebSocket, когда:
- Нужна двунаправленная связь.
- Требуется очень низкая задержка в обоих направлениях.
- Нужен бинарный протокол или сложный пользовательский протокол.

Используйте Polling, когда:
- Обновления редкие, задержка не критична.
- Нельзя держать долгоживущие соединения (ограничения инфраструктуры).
- Нужна максимально простая и статeless-модель.
...
class PerformanceComparison {
    ...
    // Гипотетические значения для иллюстрации относительных характеристик, не реальные бенчмарки.
...
- Реализуйте логику переподключения в одном месте (внутри SSE-клиента или через операторы `Flow`), избегайте дублирования.
- Храните и передавайте `Last-Event-ID`, чтобы возобновлять поток после разрыва.
- Учитывайте `retry`, отправляемый сервером, при реализации backoff.
- Интегрируйте SSE с жизненным циклом (`ViewModel`, `LifecycleOwner`), чтобы избежать утечек и лишней фоновой работы.
- Не держите все сообщения в памяти: обрабатывайте и сбрасывайте их по мере поступления.

### Резюме

- SSE подходит для уведомлений, live-счётов, котировок, дашбордов и лент активности.
- WebSocket выбирайте для чатов, игр, совместного редактирования и других сценариев с двунаправленным real-time.
- Polling уместен для нечастых запросов и ограничений инфраструктуры.
...
2. **HTTP/1.1 or HTTP/2**: Built on standard HTTP (SSE can work over HTTP/2 as a streaming response; behavior is implementation-dependent)
    ...
    fun connectWithEventSource(
        ...
        val listener = object : EventSourceListener() {
            ...
            override fun onFailure(
                ...
                // Flow completion is left to the collector's policy; re-collect to reconnect if needed.
            }
        }
...
/**
 * Parser for SSE message format (simplified, does not implement all edge cases from the spec)
 */
class SseParser {
    private var currentEvent: String? = null
    private var currentId: String? = null
    ...
    private fun parseField(line: String) {
        val colonIndex = line.indexOf(':')
        if (colonIndex == -1) {
            // Non-spec line without colon is ignored in this simplified parser
            return
        }
...
| HTTP/2 | Can be used over HTTP/2 streaming | Supported via HTTP/1.1 upgrade; HTTP/2 support depends on server/client (RFC 8441) | Limited benefit | Limited benefit |
...
class PerformanceComparison {
    ...
    // Hypothetical values to illustrate relative characteristics only; not based on benchmarks.
    suspend fun comparePerformance(): List<PerformanceMetrics> {
        val duration = 60_000L
