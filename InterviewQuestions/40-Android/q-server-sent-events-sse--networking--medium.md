...
1. **Односторонняя связь**: Сервер → Клиент только
...
6. **Текстовый формат**: Данные UTF-8 текста (обычно используется JSON)
7. **Нативная поддержка браузерами**: EventSource API в браузерах
    ...
    fun connect(
        ...
        while (shouldReconnect) {
            try {
                ...
                if (!response.isSuccessful) {
                    trySend(SseEvent.Error(Exception("HTTP ${response.code}")))
        ...
        awaitClose { shouldReconnect = false }
    }.flowOn(Dispatchers.IO)
}
```
...
| Функция | SSE | WebSocket | Long Polling | Short Polling |
|---------|-----|-----------|--------------|---------------|
| **Направление** | Сервер → Клиент | Двунаправленный | Сервер → Клиент | Сервер → Клиент |
| **Протокол** | HTTP | WebSocket | HTTP | HTTP |
| **Соединение** | Постоянное | Постоянное | Долгий запрос | Много запросов |
| **Сложность** | Низкая | Средняя | Средняя | Низкая |
| **Переподключение** | Автоматическое | Ручное | Автоматическое | Н/Д |
| **Формат** | Текст (UTF-8) | Бинарный/Текст | Любой | Любой |
| **Потребление ресурсов** | Низкое | Среднее | Среднее | Высокое |
| **Задержка** | Очень низкая | Очень низкая | Низкая | Высокая |
| **Масштабируемость** | Высокая | Средняя | Средняя | Низкая |

### Матрица решений: когда использовать каждый подход
...
```kotlin
// Пример 1: Live спортивные счета
data class ScoreUpdate(
    val matchId: String,
    val homeScore: Int,
    val awayScore: Int,
...
class StockPriceMonitor(private val typedSseClient: TypedSseClient) {

    fun monitorStocks(symbols: List<String>): Flow<TypedSseEvent<StockPrice>> {
        return typedSseClient.subscribeToEvent(
            url = "https://api.stocks.com/stream?symbols=${symbols.joinToString(",")}",
            eventType = "price-update"
        )
    }
}
...
class NotificationFeed(private val typedSseClient: TypedSseClient) {
...
1. **Всегда обрабатывайте переподключение**
   - Используйте автоматическое переподключение SSE

2. **Сохраняйте последний ID события**
   - Включите возобновление после отключения

3. **Учитывайте задержки повторных попыток**
   - Соблюдайте директиву retry сервера

4. **Мониторьте здоровье соединения**
   - Отслеживайте состояние подключения

5. **Обрабатывайте переходы фон/передний план**
   - Отключайте SSE при переходе в фон

### Распространённые ошибки

1. **Не обработка разрывов соединения**
2. **Забывание закрытия соединений**
3. **Не парсинг типов событий**
4. **Избыточное использование памяти**
5. **Не учёт времени работы батареи**

### Резюме

Server-Sent Events обеспечивают:

- **Простоту**: Построен на стандартном HTTP
- **Эффективность**: Односторонняя связь без сложности WebSocket
- **Надёжность**: Автоматическое переподключение
- **Совместимость**: Работает с существующей HTTP инфраструктурой
- **Обновления в реальном времени**: Низкая задержка push уведомлений
- **Эффективность ресурсов**: Меньше накладных расходов чем polling

**Выбирайте SSE для**: уведомлений, live счетов, цен акций, дашбордов мониторинга, лент активности
