...
- **Preferences DataStore** хранит и получает доступ к данным с помощью ключей. Эта реализация не требует предопределенной схемы и не обеспечивает типобезопасность.
- **Proto DataStore** хранит данные как экземпляры пользовательского типа данных. Эта реализация требует определения схемы с использованием protocol buffers, но обеспечивает типобезопасность.

### Хранение пар ключ-значение с Preferences DataStore

Реализация Preferences DataStore использует классы `DataStore` и `Preferences` для сохранения простых пар ключ-значение на диск.

#### Создание Preferences DataStore

```kotlin
// На верхнем уровне вашего kotlin файла:
val Context.dataStore: DataStore<Preferences> by preferencesDataStore(name = "settings")
```

#### Чтение из Preferences DataStore
...
### Хранение типизированных объектов с Proto DataStore

Реализация Proto DataStore использует DataStore и protocol buffers для сохранения типизированных объектов на диск.

#### Определение схемы

Proto DataStore требует предопределенную схему в proto файле в директории `app/src/main/proto/`:
...
Два шага для создания Proto DataStore:

1. Определите класс, реализующий `Serializer<T>`
2. Используйте property delegate созданный `dataStore`
...
Можно использовать `runBlocking()` для синхронного чтения:

```kotlin
val exampleData = runBlocking { context.dataStore.data.first() }
```

Однако выполнение синхронных I/O операций в UI потоке может вызвать ANR или UI jank. Рекомендуется асинхронно предварительно загружать данные:
